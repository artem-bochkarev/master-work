...
uint moveXa( uint x, uint direction, uint x_size, uint action )
{
    int a = (direction + 1) & 1;
    //int b = a*( (int)direction - 1 )*(1 - step( 1, action ));
    bool c = ( a == 0 )||( action != 0 );
    int b = select( (int)direction - 1, 0, (uint)c );
	return ( x_size + x + b )%x_size;
}

uint moveYa( uint y, uint direction, uint y_size, uint action )
{
    int a = direction & 1;
    //int b = a*( (int)direction - 2 )*( 1 - step( 1, action ));
    bool c = ( a == 0 )||( action != 0 );
    int b = select( (int)direction - 2, 0, (uint)c );
	return ( y_size + y + b )%y_size;
}
...
void getInput( char* out, uint x, uint y, uint direction, const __global char* map )
{
    ...
    out[0] = myMap[x1 + x_size*y1];
    out[1] = myMap[ moveX( x1, left( direction ), x_size ) + x_size*moveY( y1, left( direction ), y_size ) ];
    out[2] = myMap[ moveX( x1, right( direction ), x_size ) + x_size*moveY( y1, right( direction ), y_size ) ];
    out[3] = myMap[ moveX( x1, direction, x_size ) + x_size*moveY( y1, direction, y_size ) ];
}
...
float run( __local char* ind, const uint statesCount, const uint stateSize, __global char* map )
{
    ...
    for ( uint i=0; i<100; ++i )
    {
        ...
        x = moveXa( x, direction, x_size, action );
        y = moveYa( y, direction, y_size, action );
        cnt += myMap[x + x_size*y];
        myMap[x + x_size*y] = 0;
        direction = actionTurn( direction, action );
    }
...
{
    //make copy of a map
    uint x_size = (uint)maps[0];
    uint y_size = (uint)maps[1];
    for ( uint i=0; i< x_size*y_size+2; ++i )
    {
        //myMaps[i] = maps[i];
        myMaps[i] = maps[i];
    }
