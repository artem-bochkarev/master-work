...
void getInput( char* out, uint x, uint y, uint direction, const __global char* map )
{
    ...
    out[1] = myMap[ moveX( x1, left( direction ) ) + x_size*moveY( y1, left( direction ) ) ];
    out[2] = myMap[ moveX( x1, right( direction ) ) + x_size*moveY( y1, right( direction ) ) ];
    out[3] = myMap[ moveX( x1, direction ) + x_size*moveY( y1, direction ) ];
}
